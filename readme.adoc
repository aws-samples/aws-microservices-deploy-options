...
```
+
If the output is empty, this means the role does not exist and needs to be created. Create the role:
+
```
aws iam create-role \
  --role-name ecsInstanceRole \
  --assume-role-policy ecs-assume-role-policy.yaml
aws iam put-role-policy \
  --role-name ecsInstanceRole \
  --policy-name MyPolicy \
  --policy-document ecs-instance-role-policy.yaml
```
+
