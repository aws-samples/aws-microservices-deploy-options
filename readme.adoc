...
```
+
This will be used during cluster creation.
+
...
```
+
.. Check if the `ecsInstanceRole` already exists:
+
```
...
a| image::./images/deploy-to-aws.png[link=https://console.aws.amazon.com/cloudformation/home?region=us-east-1#/stacks/new?stackName=my-ecs-cluster&templateURL=https://s3.amazonaws.com/aws-microservices-deploy-options/ecs-cluster.yaml]
|===

You would need to specify the VPC, the subnet and the security group. Everything else has a default value.

The CloudFormation template used for this is also stored at `config/ecs/ecs-cluster.yaml`.
