...
=== Deployment: Canary Deployment of Services (work in progress)
...
. Create route rules:
  ...
  kubectl apply -f apps/k8s/canary/route-50-50.yaml
...
Another route with the traffic split of 90% and 10% is at `apps/k8s/canary/route-90-10.yaml`.
...
. Prometheus addon will obtain the metrics from Istio. Install Prometheus:
  ...
  istio-0.7.1 $ kubectl apply -f install/kubernetes/addons/prometheus.yaml
...
. Install the Servicegraph addon; Servicegraph queries Prometheus, which obtains details of the mesh traffic flows from Istio:
  ...
  istio-0.7.1 $ kubectl apply -f install/kubernetes/addons/servicegraph.yaml
