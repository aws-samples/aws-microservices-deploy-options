...
This section will explain how to deploy Fargate services using AWS CLI. It will use an ECS cluster created using CloudFormation as explained at link:#deployment-create-cluster-using-aws-cloudformation[].

Get output from the CloudFormation stack:
...
.. `greeting` service:
  ...
  aws ecs create-service \
    ...
    --network-configuration "awsvpcConfiguration={subnets=[<private-subnet-name>],securityGroups=<sg-namehere>}"
...
.. `webapp` service:
  ...
  aws ecs create-service \
    ...
    --health-check-grace-period-seconds 60 \
    --network-configuration "awsvpcConfiguration={subnets=[<public-subnet-name>],securityGroups=<sg-namehere>}"
