...
To push the code into the CodeCommit repo created in the CloudFormation stack. Please setup your Git credential by following the https://docs.aws.amazon.com/codecommit/latest/userguide/setting-up-https-unixes.html[document].

When the Git credential is setup, you can use the following command to push in the code and trigger the pieline to run.

. `git push codecommit master`

Your can access to the pipeline via AWS console using the following command to get its URL
...
Example of the screenshot in AWS console when the pipeline is created
