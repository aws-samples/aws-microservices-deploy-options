...
. Compile final package: `pack up -f .`
. Install package: `./manifests/output/install.sh`
. Access the application: `curl http://$(kubectl get svc/webapp -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')`
. Delete the application: `kubectl delete -R -f manifests/output/github.com/kubepack/aws-microservices-deploy-options/apps/k8s/kubepack`
