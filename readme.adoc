...
. `cd apps/lambda`
. `sam local start-api`
. `curl http://127.0.0.1/resources/greeting`
. `curl http://127.0.0.1/resources/names`
. `curl http://127.0.0.1/resources/names/1`
. `curl http://127.0.0.1/`

Test SAM on AWS:

# TODO: provide command to get stack output of the link

. `aws cloudformation describe-stack --stack-name aws-compute-options-lambda --query "Stacks[].Outputs[?\`\`"]`
