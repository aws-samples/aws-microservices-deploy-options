...
2. Run the following script to setup the underlying infrastructure:
...
a| image::./images/deploy-to-aws.png[link=https://console.aws.amazon.com/cloudformation/home?region=us-east-1#/stacks/new?stackName=aws-compute-options-ecscli&templateURL=https://s3.amazonaws.com/compute-options-public/ecscli/infra.yaml]
|===
+
...
+
3. Inspect the contents of `ecs-cluster.prop` and source this property file:
...
5. Generate the `ecs-params.yml` for the `greeting` service:
    
    ecs-params-create.sh greeting

6. Bring the `greeting` service up:
...
7. Run this command to check the health of the `GreetingTargetGroupArn`:
    
    aws elbv2 describe-target-health --target-group-arn $GreetingTargetGroupArn 

8. Generate the `ecs-params.yml` for the `name` service:

    ecs-params-create.sh name

9. Bring the `name` service up:

    ecs-cli compose --verbose \
      --file name-docker-compose.yaml \
      --task-role-arn $ECSRole \
      --ecs-params ecs-params_name.yml  \
      service up \
      --target-group-arn $NameTargetGroupArn \
      --container-name  name-service \
      --container-port 8082

10. Run this command to check the health of the `NameTargetGroupArn`:

    aws elbv2 describe-target-health --target-group-arn $NameTargetGroupArn

11. Generate the `ecs-params.yml` for the `webapp` service:

    ecs-params-create.sh webapp

12. Change the `GREETING_SERVICE_HOST` and `NAME_SERVICE_HOST` to the `PrivateALBCName` property (Get it from the ecs-cluster.prop a directory up or in CloudFormation Outputs tab)

13. Bring the webapp service up

    ecs-cli compose --verbose \
      --file webapp-docker-compose.yaml \
      --task-role-arn $ECSRole \
      --ecs-params ecs-params_webapp.yml \
      service up \
      --target-group-arn $WebappTargetGroupArn \
      --container-name webapp-service \
      --container-port 8080

14. Run this command to check the health of the `WebappTargetGroupArn`:

    aws elbv2 describe-target-health --target-group-arn $WebappTargetGroupArn
