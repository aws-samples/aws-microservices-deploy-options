...
|===
+
. Run the command to capture the output from the CloudFormation template. This command will generate the file `ecs-cluster.props`. It consists of the key/value pairs from the Output of the CloudFormation template. These will be used to setup environment variables which are used subseqently.
...
. Run this command to check the `healthy` status of the `greeting` service:
+   
...
. Run this command to check `healthy` status of the `name` service:
...
. Run this command to check `healthy` status of the `webapp` service:
...
. Once all services are in `healthy` state, get response from the service:
...
ecs-cli compose --verbose \
      ...
      --ecs-params ecs-params_webapp.yaml \
      service down
aws cloudformation delete-stack --stack-name aws-microservices-deploy-options-ecscli
```
