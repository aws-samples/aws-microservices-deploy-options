...
.. Create pipeline for `greeting`, `name` and `webapp` services. Users please following the instruction in the (https://docs.aws.amazon.com/codepipeline/latest/userguide/GitHub-rotate-personal-token-CLI.html)[document] to generate GitHut personal access token. Note: user can override the defualt repository to forked public repo by providing URL to the parameter `Git`

  cd app/lambda
  aws cloudformation deploy \
    --template-file microservice-pipeline.yaml \
    --stack-name aws-compute-options-greeting-lambda-pipeline \
    --parameter-overrides ServiceName=greeting GitHubOAuthToken=XXXXX \
    --capabilities CAPABILITY_IAM
  aws cloudformation deploy \
    --template-file microservice-pipeline.yaml \
    --stack-name aws-compute-options-name-lambda-pipeline \
    --parameter-overrides ServiceName=name GitHubOAuthToken=XXXXX \
    --capabilities CAPABILITY_IAM

when both `greeting` and `name` services are created successfully, user can deploy the `webapp` service by using the following command

  aws cloudformation deploy \
    --template-file microservice-pipeline.yaml \
    --stack-name aws-compute-options-webapp-lambda-pipeline \
    --parameter-overrides ServiceName=webapp GitHubOAuthToken=XXXXX \
    --capabilities CAPABILITY_IAM
