...
This section will use a cluster previously created.
...
.. Create the `webapp` service. For that, get the public subnet and security group:
+
```
  aws cloudformation \
    describe-stacks \
    --region us-east-1 \
    --stack-name MyECSCluster \
    --query 'Stacks[].Outputs[?OutputKey==`PublicSubnet1`].[OutputValue]' \
    --output text
  aws cloudformation \
    describe-stacks \
    --region us-east-1 \
    --stack-name MyECSCluster \
    --query 'Stacks[].Outputs[?OutputKey==`SecurityGroupWebapp`].[OutputValue]' \
    --output text
```
+
And, finally create the `webapp` service. Make sure to replace `<cluster-name>`, `<webapp-task>`, `<subnet>` and `<security-group>`:
+
```
aws ecs create-service \
      --cluster <cluster-name> \
      ...
      --desired-count 1 \
      --launch-type "FARGATE" \
      --network-configuration "awsvpcConfiguration={subnets=[<subnet>],securityGroups=[<security-group>]}"
```
