...
. *Docker image name*: By default, the Docker image name is `arungupta/<service>-war`. Create the image in your repo as: `mvn -pl <service> package -Pdocker -Ddocker.repo=<repo>`
...
. Create Helm chart: `helm install --name myapp apps/k8s/myapp`
...
For local testing and deploying to AWS Lambda and Amazon API Gateway refer to the README file in the link:tree/master/apps/lambda[`apps/lambda` folder].
