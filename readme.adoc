...
. Kubernetes cluster must have been created with RBAC enabled. If it is, then skip this step. Otherwise, create the Kubernetes cluster:
+
```
kops create cluster \
...
```
+
. Pipelines need to use a docker registry running inside the kubernetes cluster. Configure the Kubernetes cluster with insecure registries:
+
```
kops edit cluster
```
...
```
+
. Install Jenkins X on the Kubernetes cluster:
+
```
jx install --provider=aws
```
+
Follow the prompts until you see the message:
+
```
